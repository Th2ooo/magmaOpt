...
def iniWF():
  ...
  setAtt(file=path.EXCHFILE,attname="Observation",attval=path.REFTARG)
  ...
  setAtt(file=path.EXCHFILE,attname="MeshTarget",attval=path.MESHTARG)
  setAtt(file=path.EXCHFILE,attname="SolTarget",attval=path.SOLTARG)
...
######                    J (real) value of objective                      ######
#################################################################################

def printHisto(it,J):

  histo = open(path.HISTO,'a')
  print("{} {}".format(it,J),file=histo)
  histo.close()
